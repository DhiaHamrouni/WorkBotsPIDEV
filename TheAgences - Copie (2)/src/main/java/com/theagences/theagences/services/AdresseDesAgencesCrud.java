...
public class AdresseDesAgencesCrud {
    ...
    public void ajouterAdresse2(AdresseDesAgences AD) {
        try {
            String requete2 = "INSERT INTO adresse_des_agences (region ,avenue ,n_rue,code_postal) " + "VALUES (?,?, ? ,?)";
    ...
    public void ajouterAdresse3(AdresseDesAgences AD) {
        try {
            String requete2 = "INSERT INTO adresse_des_agences (region ,avenue ,n_rue,code_postal) " + "VALUES (?,?, ? ,?)";
            PreparedStatement pst = new MyConnection().getCnx().prepareStatement(requete2);
            pst.setString(1, AD.getRegion());
    ...
    public void ModifierAgence(AdresseDesAgences D) {
        ...
        try {
            ...
            String requete3 = "UPDATE `adresse_des_agences` SET `id_adresse`= ? ,`region`=?,`avenue`=?,`n_rue`= ?,`code_postal`=? WHERE id_adresse= ?";
